...
export function buildDashboard(r: ConnectorResult): DashboardData {
  ...
  // Closed Deals (and the close-rate denominator) always come from Closer
  // EOD — that's the closer-activity truth and keeps Close Rate sensible.
  const closes = r.sales.reduce(
    (a, s) => a + s.oneCallCloses + s.followUpCloses,
    0,
  );
  // Cash / revenue ALWAYS come from the payment logs (Stripe + Whop + OTO).
  // The closer's self-reported cash field is kept for the per-closer table
  // only — closers sometimes log payment-plan installment cash without a new
  // close, which made Cash and Closed Deals contradict each other before.
  const buyers = r.buyers;
  const buyerCash = buyers.reduce((a, b) => a + b.amount, 0);
  const cashCollected = buyerCash;
  const revenue = buyerCash;
  ...
  // EOD per day → closer-activity (booked / taken / closes)
  ...
  // Payment logs per day → real cash and revenue.
  for (const b of buyers) {
    if (!b.date) continue;
  ...
  const totalBuyers = new Set(buyers.map((b) => b.email).filter(Boolean)).size;
