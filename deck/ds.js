...
function pickAccent(text, override, pats){
  ...
  for (const p of (pats||PATS)){ const m = masked.match(p); if (m) return [m.index, m.index+m[0].length]; }
  return null;
}
...
module.exports = { C, FONT, LEX, PATS, RAIL, RIGHT, CW, TIERS, T, nlines, blockH, runs, pickAccent,
