...
for (const [b,s] of Object.entries(DIV)){
  const t = paras(s)[0];                       // "STEP 1: THE TIME BASED RANGES"
...
function kChat(g){
  ...
  const psz = 17;
  const pw = Math.max(2.60, Math.min(7.40, pill.t.length * 0.0072 * psz + 0.80));
  const py = rest.length ? 3.70 : 2.45;
  blocks.forEach((_,i) => {
    const s = newSlide(g,false);
    ...
    if (pill.frag || blocks[i].some(x => x.t === pill.t)){
      s.addShape("roundRect", { x:(10-pw)/2, y:py, w:pw, h:0.62, rectRadius:0.31, fill:{ color:C.accent } });
      s.addText(pill.t, T({ x:(10-pw)/2, y:py, w:pw, h:0.62, fontSize:psz, color:C.white, align:"center" }));
    }
  });
...
pres.writeFile({ fileName: "DTR_Intro_Content_designed.pptx" }).then(() => {
