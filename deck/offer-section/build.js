...
const { C, T, CW, RAIL, TIERS, nlines, runs, chapterBanner, eyebrow, accentDot, textW } = D;
...
function resolveAccents(flat, max, noFallback){
  const map = new Map();
  const hits = [];
  for (const l of flat){
    const ov = accOf(l);
  ...
  if (hits.length === 0 && !noFallback){        // still nothing — one short line carries the colour
...
function kList(g){
  ...
  const A = resolveAccents(flat, 0, blocks.flat().some(l => NUMRE.test(l.t)));
  ...
  const numSz = r => Math.round(r * 1.15);
  const hasNum = rows.some(r => r.num);
  let numW = 0.30;
  if (hasNum) numW = Math.min(1.70, Math.max(...rows.filter(r=>r.num)
    .map(r => textW(r.num, numSz(19)))) + 0.10);
  const bx = hasNum ? RAIL + numW + 0.22 : 1.25, bw = 9.10 - bx;
  const headGap = head ? 0.38 : 0;
  let rsz = 22;
  for (;;){
    ...
    if (tot <= 5.30 - (top + hh + headGap) || rsz <= 15){
      // centre heading and rows together, so a short list doesn't hug the top
      const block = hh + headGap + tot;
      hy = top + Math.max(0, (5.30 - top - block)/2);
      let y = hy + hh + headGap;
  ...
  blocks.forEach((_,bi) => {
    ...
    rows.forEach(r => {
      ...
      if (r.num){
        s.addText(r.num, T({ x:RAIL, y:r.y, w:numW + 0.14, h:Math.min(r.h,0.55),
          fontSize:numSz(r.size), color:C.accent, valign:"top" }));
      } else {
...
let seen = 0;
for (const g of PLAN2) if (g.k === "stack"){ g.prevCount = seen; seen = g.stack.length; }


// ---------- the offer stack ------------------------------------------------
// Modelled on the reference deck's recurring stack slide: a dark frame that
// returns every time something is added, one more node lit each time. Row
// positions are fixed for the full nine slots, so nothing ever moves.
const CODE = /^((?:BONUS\s*\d+\s*:)|(?:\d+\s*[.)\-]))\s*(.+)$/s;
const STACK_PITCH = 0.46, STACK_ROW = 0.35;
const SPINE_X = 0.80, CODE_X = 1.00, CODE_W = 1.20, LABEL_X = 2.35, LABEL_W = 6.75;

function kStack2(g){
  const s = newSlide(g, true);
  const rows = g.stack.map(n => {
    const t = paras(n)[0];
    const m = t.match(CODE);
    return { code: m ? m[1] : "", label: m ? m[2] : t, src:n };
  });
  const k = rows.length;
  const newest = (g.stack.length > (g.prevCount|0)) ? k-1 : -1;
  // every label on one line, at the largest size that allows it
  let fs = 15;
  while (fs > 11 && rows.some(r => nlines(r.label, fs, LABEL_W) > 1)) fs -= 1;
  // these slides are minutes apart, never consecutive, so centre each one
  const total = (k-1)*STACK_PITCH + STACK_ROW;
  const y0 = (5.625 - total)/2;
  const yOf = i => y0 + i*STACK_PITCH;

  if (k > 1) s.addShape("line", { x:SPINE_X, y:yOf(0)+STACK_ROW/2, w:0,
    h:(k-1)*STACK_PITCH, line:{ color:C.darkRule, width:2 } });

  rows.forEach((r,i) => {
    const y = yOf(i), cy = y + STACK_ROW/2, on = (i === newest);
    if (on){
      s.addShape("ellipse", { x:SPINE_X-0.15, y:cy-0.15, w:0.30, h:0.30, fill:{ color:C.accent } });
    } else {
    ...
    s.addText(r.code,  T({ x:CODE_X, y:y, w:CODE_W, h:STACK_ROW, fontSize:fs,
                           color:C.accent, align:"right" }));
    s.addText(r.label, T({ x:LABEL_X, y:y, w:LABEL_W, h:STACK_ROW, fontSize:fs,
                           color: on ? C.accent : C.white }));
  });
  const notes = g.src.map(n => paras(n)[0]).concat(g.note ? [paras(g.note)[0]] : []);
  s.addNotes(notes.join("\n\n"));
  notes.forEach(t => say(t));
}

// ---------- an offer component's own slide ---------------------------------
function kItem(g){
  const s = newSlide(g, false);
  accentDot(s);
  const t = paras(g.src[0])[0];
  ...
  if (code) s.addText(code, T({ x:RAIL, y:1.62, w:CW, h:0.42, fontSize:24, color:C.accent, valign:"top" }));
  let tz = 40;
  while (tz > 22 && nlines(title, tz, CW) * tz * 1.24/72 > 1.90) tz -= 2;
  const th = nlines(title, tz, CW) * tz * 1.24/72;
  s.addText(runs(title, tz, C.ink, undefined),
    T({ x:RAIL, y:2.20, w:CW, h:th, fontSize:tz, valign:"top" }));
  say(t);
}
...
function kHero(g){
  ...
  const t = paras(g.src[0])[0];
  const m = t.match(/^(.*?[-–—])\s*(.+)$/s);
  const kicker = m ? m[1] : "", title = m ? m[2] : t;
  if (kicker) s.addText(kicker, T({ x:RAIL, y:1.72, w:CW, h:0.36, fontSize:13, color:C.accent, valign:"top" }));
  let tz = 46;
  while (tz > 28 && nlines(title, tz, CW) * tz * 1.24/72 > 2.10) tz -= 2;
  const th = nlines(title, tz, CW) * tz * 1.24/72;
  s.addText(title, T({ x:RAIL, y:2.25, w:CW, h:th, fontSize:tz, color:C.ink, valign:"top" }));
  say(t);
}

const K = { stmt:kStack, build:kStack, num:kList, list:kList, chat:kChat,
            cue:kCue, word:kWord, div:kDiv, pair:kPair,
            stack:kStack2, item:kItem, hero:kHero };
