...
(async()=>{
  ...
  for(const t of [...set]){
    const m=t.match(/^((?:Q\d+\s*[-–—:])|(?:\d+\s*[.\-)]))\s*(.+)$/s); if(m){ set.add(m[1]); set.add(m[2]); }
    const c=t.match(/^((?:BONUS\s*\d+\s*:)|(?:\d+\s*[.)\-]))\s*(.+)$/s); if(c){ set.add(c[1]); set.add(c[2]); }
    const h=t.match(/^(.*?[-–—])\s*(.+)$/s); if(h){ set.add(h[1]); set.add(h[2]); }
